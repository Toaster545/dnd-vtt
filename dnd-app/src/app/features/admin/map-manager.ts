...
export class MapManagerComponent implements OnInit {
  ...
  private async loadMaps() {
    ...
    try {
      this.maps.set(await this.mapService.getMapsForCampaign(CAMPAIGN_ID));
    } catch (e: any) {
  ...
  onFileChange(event: Event) {
    ...
    if (this.selectedFile && !this.newMapName) {
      this.newMapName = this.selectedFile.name.replace(/\.[^.]+$/, '');
    }
  }
